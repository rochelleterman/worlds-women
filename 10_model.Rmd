...
```{r}
# MUSLIM MAJORITY
logit.r8.1 <- glm(n.binary ~ n.binary.lag + count + women_composite*muslim.maj + polity2 + domestic9 + log(pop.wdi) + log(gdp.pc.un) + as.factor(year), data = rt, family = binomial(link="probit"), control = list(maxit = 50))
summary(logit.r8.1)
...
# MENA
logit.r8.2 <- glm(n.binary ~ n.binary.lag + count + women_composite*mena + polity2 + domestic9 + log(pop.wdi) + log(gdp.pc.un) + as.factor(year), data = rt, family = binomial(link="probit"), control = list(maxit = 50))
logit.r8.2.se <- coeftest(logit.r8.2, vcov=function(x) vcovHC(x, cluster="group", type="HC1"))[,2]

# PERCENT MUSLIM
logit.r8.3 <- glm(n.binary ~ n.binary.lag + count + women_composite*muslim + polity2 + domestic9 + log(pop.wdi) + log(gdp.pc.un) + as.factor(year), data = rt, family = binomial(link="probit"), control = list(maxit = 50))
